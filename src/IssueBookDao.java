...
public static int save(String bookcallno,int studentid,String studentname,String studentcontact, String returndate){
	int status=0;
	try{
		Connection con=DB.getConnection();
		...
		if(status>0){
		PreparedStatement ps=con.prepareStatement("insert into issuebooks(bookcallno,studentid,studentname,studentcontact, returndate) values(?,?,?,?,?)");
		ps.setString(1,bookcallno);
